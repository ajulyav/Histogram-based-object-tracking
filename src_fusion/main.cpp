...
int main(int argc, char ** argv)
{
	...
	std::string output_path = "/home/avsa/AVSA2020results/Ex3";										//location to save output videos
	...
	for (int s=0; s<NumSeq; s++ )
	{
		...
		for (;;) {
			...
			if(frame_idx == 1)
			{
				...
				modelImage = one_channelFrame(objectModel);

				//the model - histogram
				basicModelHistogram_color = get_ColorHistogram(modelImage, hist_bins);
				basicModelHistogram_gradient = get_GradientHistogram(modelImage, hist_bins);
			...
			std::vector<Mat> candidates_color, candidates_gradient; // histogram of the candidates
			std::vector<Rect> colorcandidate_rectangle, gradientcandidate_rectangle; //position of candidates
			std::vector<float> distances_color, distances_gradient;

			int index_best_distance_color,index_best_distance_gradient,index_with_smallest_distance_combined;

			//Computing the histogram and location
			get_candidates(one_channelFrame, candidates_color, colorcandidate_rectangle, currentState, size_nghbd, step, hist_bins,true);
			get_candidates(one_channelFrame, candidates_gradient, gradientcandidate_rectangle, currentState, size_nghbd, step, hist_bins,false);

			cout << "Number Of Candidates: " << candidates_color.size() << endl;

			//the best candidate
			index_best_distance_color = Battacharyya_distance(basicModelHistogram_color, candidates_color, distances_color, true);
			index_best_distance_gradient = Battacharyya_distance(basicModelHistogram_gradient, candidates_gradient, distances_gradient, false);
			index_with_smallest_distance_combined = bestCombinedCandidate(distances_color, distances_gradient);

			cout<<endl;
			cout<<"frame NO. "<< frame_idx << " : the "<<index_best_distance_color<<"th color candidate,  "<<"the "<<index_best_distance_gradient<<"th gradient candidate"<<endl;
			cout<< "the "<<index_with_smallest_distance_combined<<"th combined candidate"<<endl;

			list_bbox_est.push_back(colorcandidate_rectangle[index_with_smallest_distance_combined]);

			//plot the histograms
			cv::Mat plotHistograms = plot_histogram(basicModelHistogram_color, candidates_color[index_with_smallest_distance_combined], hist_bins);
			...
			ShowManyImages("Tracking", 4, frame, frame(list_bbox_est[frame_idx-1]),modelImage, plotHistograms);
