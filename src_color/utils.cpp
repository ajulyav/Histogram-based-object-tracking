...
cv::Mat calculate_RegionHist(Mat cropped_frame, int hist_bins){
	Mat tempHistogram;
	Mat model_histogram;
	float range[] = { 0, 256 };
	...
	normalize(tempHistogram, model_histogram, 0, 1, NORM_MINMAX, -1, Mat() );
...
//hist_bins - number of bins in the histogram
void get_candidates(const Mat one_channelFrame,
	...
	//check if window is out of the frame
	...
	for(int i = startx; i < endx+1; i+=step ){
		for(int j = starty; j < endy+1; j+=step){
			...
			if( i >= 0 && (i + currentState.width) <=  one_channelFrame.cols && j >= 0 && (j + currentState.height) <=  one_channelFrame.rows  ){
			    ...
			    singleCandidate =calculate_RegionHist(cropped, hist_bins);
...
int Battacharyya_distance(Mat model_histogram, std::vector<Mat> candidates)
{
	...
	double smallest_distance = compareHist(model_histogram, candidates[0], CV_COMP_BHATTACHARYYA);

	for(int i = 1; i < candidates.size(); i++)
	{
		double current_distance = compareHist(model_histogram, candidates[i], CV_COMP_BHATTACHARYYA);
