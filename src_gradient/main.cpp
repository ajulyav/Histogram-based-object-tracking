...
int main(int argc, char ** argv)
{
	...
	std::string output_path = "/home/avsa/AVSA2020results/Ex2";										//location to save output videos
	...
	std::string sequences[] = {"basketball"};
	...
	for (int s=0; s<NumSeq; s++ )
	{
		...
		for (;;) {
			...
			if(frame_idx == 1)
			{
				...
				//the model (histogram)
				model_histogram = gradient_Histogram(model_img,hist_bins);
			...
			list_bbox_est.push_back(candidate_rectangle[index_best_distance]);
